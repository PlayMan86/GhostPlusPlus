	...
	for( vector<PairedWPSCheck> :: iterator i = m_PairedWPSChecks.begin( ); i != m_PairedWPSChecks.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			...
			if( W3MMDPlayerSummary && W3MMDPlayerSummary->GetTotalGames( ) > 0 )
			{
				...
				else if( Category == "hvsa" ) CategoryName = "Horde Vs Alliance X3";
				else if( Category == "hvsatest" ) CategoryName = "Horde Vs Alliance X3";
				
				int WinChance = (100 * W3MMDPlayerSummary->GetTotalWins( ) / W3MMDPlayerSummary->GetTotalGames( ));
				
				string Summary = "[" + StatsName + "] has played " + UTIL_ToString( W3MMDPlayerSummary->GetTotalGames( ) ) + " " + CategoryName + " games here (ELO: " + UTIL_ToString( W3MMDPlayerSummary->GetScore( ), 2 ) + "; rank: " + UTIL_ToString( W3MMDPlayerSummary->GetRank( ) ) + "). W/L: " + UTIL_ToString( W3MMDPlayerSummary->GetTotalWins( ) ) + "/" + UTIL_ToString( W3MMDPlayerSummary->GetTotalLosses( ) ) + ". Win chance: " + UTIL_ToString(WinChance) + "%.";
...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
	...
	if( ( player->GetSpoofedRealm( ) == "europe.battle.net" ) && User == "playman" )
	  RootAdminCheck = true;
	...
	*********************/
	...
	else if( (Command == "castlestats" || Command == "cfstats" || Command == "cfs" ) && GetTime( ) - player->GetStatsDotASentTime( ) >= 5 )
	{
        ...
        else if( (Command == "hvsastats" || Command == "hvastats" || Command == "hvsas" || Command == "hvsa" ) && GetTime( ) - player->GetStatsDotASentTime( ) >= 5 )
        {
                string StatsUser = User;
                ...
                if( player->GetSpoofed( ) && ( AdminCheck || RootAdminCheck || IsOwner( User ) ) )
                        m_PairedWPSChecks.push_back( PairedWPSCheck( string( ), m_GHost->m_DB->ThreadedW3MMDPlayerSummaryCheck( StatsUser, StatsRealm, "hvsa" ) ) );
                else
                        m_PairedWPSChecks.push_back( PairedWPSCheck( User, m_GHost->m_DB->ThreadedW3MMDPlayerSummaryCheck( StatsUser, StatsRealm, "hvsa" ) ) );
        ...
        else if( (Command == "hvsateststats" || Command == "hvateststats" || Command == "hvsats" ) && GetTime( ) - player->GetStatsDotASentTime( ) >= 5 )
        {
                string StatsUser = User;
                ...
                if( player->GetSpoofed( ) && ( AdminCheck || RootAdminCheck || IsOwner( User ) ) )
                        m_PairedWPSChecks.push_back( PairedWPSCheck( string( ), m_GHost->m_DB->ThreadedW3MMDPlayerSummaryCheck( StatsUser, StatsRealm, "hvsatest" ) ) );
                else
                        m_PairedWPSChecks.push_back( PairedWPSCheck( User, m_GHost->m_DB->ThreadedW3MMDPlayerSummaryCheck( StatsUser, StatsRealm, "hvsatest" ) ) );
